...
<!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Rejected Groups</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Rejected Groups</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
...
<script>
    ...
    $(document).ready(function(){
        var table = $('#example1').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url('/admin/group/rejectedgrouplist') }}", // Updated URL
                type: 'POST',
                headers: {
    ...
    function idleGroup(id) {
        Swal.fire({
            title: 'Are you sure?',
            text: "Do you want to make idle this group?",
            icon: 'success',
            showCancelButton: true,
            confirmButtonColor: '#333',
            cancelButtonColor: '#3085d6',
            confirmButtonText: 'Yes, idle it!'
        }).then((result) => {
            if (result.isConfirmed) {
                
                $.ajax({
                    url: '/admin/group/idle/' + id,  
                    type: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}'  
                    },
                    ...
                    error: function(xhr, status, error) {
                        Swal.fire(
                            'Error!',
                            'There was an issue to idle the group.',
                            'error'
                        );
                    }
                });
