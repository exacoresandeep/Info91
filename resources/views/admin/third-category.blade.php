...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0">Third Category</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Third Category</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    ...
    <section class="content">
        ...
        <div class="add-button-container d-flex justify-content-between align-items-center">
            <div class="row">
                ...
                <div class="col-lg-6 d-flex align-items-center" >                
                    <select class="form-control" id="secondCategorySelect">
    ...
    <div class="modal fade" id="addCategoryModal" tabindex="-1" role="dialog" aria-labelledby="addCategoryModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="addCategoryModalLabel">Add Third Category</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
<script>
   ...
   $(document).ready(function () {
        ...
        $('#firstCategorySelect').on('change', function () {
            $('#secondCategorySelect').val("");
            const firstCategoryId = $(this).val();
            $.ajax({
                url: "{{ route('admin.secondCategoryLists') }}", // Adjust route
                method: 'GET',
                data: {
                    first_category_id: firstCategoryId // Passing the selected first category ID
                },
                success: function (response) {
                    if (response.success) {
                        let options = '<option value="">Select Second Category</option>';
                        response.data.forEach(category => {
                            options += `<option value="${category.id}">${category.name}</option>`;
                        });
                        $('#secondCategorySelect').html(options);
                    } else {
                        Swal.fire('Error', response.message || 'Could not load categories.', 'error');
                    }
                },
                error: function () {
                    Swal.fire('Error', 'Failed to load second categories.', 'error');
                }
            });
            $('#example1').DataTable().ajax.url("{{ url('/admin/thirdCategoryList') }}?first_category="+firstCategoryId).load();
        });

        $('#secondCategorySelect').on('change', function () {
            const secondCategoryId = $(this).val();
            $('#example1').DataTable().ajax.url("{{ url('/admin/thirdCategoryList') }}?second_category_id=" + secondCategoryId).load();
        });

        $('#loadCreateForm').on('click', function() {
            const url = "{{ route('admin.third-category.create') }}";
        ...
        var table = $('#example1').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ url('/admin/thirdCategoryList') }}", 
                type: 'POST',
                headers: {
            ...
            columns: [
                ...
                { data: 'third_category_name', name: 'third_category_name' },
    ...
    function saveCategoryButton() {
        ...
        var formData = {"third_category_name":$("#third_category_name").val(),
            "second_category_id":$("#second_category_id").val(),
            "status":"1"}

        $.ajax({
            url: "{{ url('/admin/addThirdCategory') }}", // Update the route as necessary
            type: "POST",
            data: formData,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    function handleAction(itemId, action) {
        $.ajax({
            url: '/admin/viewThirdCategory/' + itemId,
            method: 'GET',
            success: function(response) {
                if (action === 'view') {
    ...
    function editThirdCategory(itemId) {
        $.ajax({
            url: '/admin/editThirdCategory/' + itemId, 
            method: 'GET',
            success: function(response) {
                if (response.success) {
    ...
    function updateCategory(itemId) {
        ...
        $.ajax({
            url: '/admin/updateThirdCategory/' + itemId, 
            method: 'POST',
            data: formData,
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    ...
    function deleteThirdCategory(itemId) {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/admin/deleteThirdCategory/' + itemId,
                    method: 'DELETE',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
